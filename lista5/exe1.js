...
function maiorNota(vetObj){
    ...
    let objeMaiorNota = vetObj[0]
    for(let i=0; i<vetObj.length; i++){
        if(vetObj[i].nota > objeMaiorNota.nota){
            objeMaiorNota = vetObj[i]       
        }
}
console.log(objeMaiorNota)
}

function media(vetObj){
    console.log(`A media de notas de todos os alunos é`)
    let soma = 0
    for(let i=0; i<vetObj.length; i++){
        soma += vetObj[i].nota
    }
...
function principal(){
    //variavel alunos tem escopo - existe somente dentro da função principal
    let alunos = [
